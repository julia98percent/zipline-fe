...
import { Customer, CustomerUpdateData, Label } from "@/types/customer";
...
const CustomerDetailPage = () => {
  ...
  const fetchCustomerData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const customerData = await fetchCustomerDetail(customerUid);
  ...
  const handleSaveEdit = async () => {
    if (!editedCustomer || !customerUid) return;
    ...
    try {
      ...
      if (typeof customerUid !== "string") return;
      await updateCustomer(customerUid, requestData);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deleteCustomer(customerUid);
      showToast({
        message: "고객을 삭제했습니다.",
        type: "success",
      });
  ...
  return (
    <CustomerDetailView
      loading={loading}
      customer={customer}
      customerUid={typeof customerUid === "string" ? customerUid : undefined}
      isEditing={isEditing}
